...
def test_register_account(logic, prepare_user):
    ...
    response = logic.account_api.graphql.register_account(email=email, login=login, password=password)
...
def test_register_account_with_exists_login(logic):
    ...
    with message_checker(error_message=error_message):
        logic.account_api.graphql.register_account(email=email, login=login, password=password)
